...
> [!TIP]
> Find connected notes with `Ctrl+K`. Link related topics in the Connections section to build your knowledge graph.
...
```mermaid
flowchart LR
    A[AI Summary] --> B{Worth keeping?}
    B -->|Yes| C[Distill Core Insight]
    B -->|No| D[/Discard/]
    C --> E[Find Connections]
    E --> F[Link to Existing Notes]
    F --> G([Permanent Knowledge Note])
```
